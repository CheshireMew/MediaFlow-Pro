...
def _project_snapshot(project) -> dict[str, Any]:
    return {
        ...
        "sequences": [item.model_dump(mode="json") for item in project.list_sequences()],
        "assets": [item.model_dump(mode="json") for item in project.list_assets()],
        "web_assets": [item.model_dump(mode="json") for item in project.list_web_assets()],
        "active_workflows": [
            item.model_dump(mode="json") for item in project.list_workflow_runs(active_only=True)
        ],
        "tasks": [item.model_dump(mode="json") for item in project.list_tasks()],
    }


def _sequence_id(project, arguments: dict[str, Any]) -> str:
    return str(arguments.get("sequence_id") or project.get_project().main_sequence_id)
...
def _task_idempotency(envelope: AutomationRequest, operation: str) -> str | None:
    if not envelope.request_id:
        return None
    return f"automation:{envelope.request_id}:{operation}"


def _dispatch_project_request(
    project: Any,
    ...
    if operation == "project.version.list":
        return {"versions": [item.model_dump(mode="json") for item in project.list_versions()]}
    if operation == "project.version.restore":
        record = project.restore_version(str(_required(arguments, "version_id")))
        return {
            "restored_version": record.model_dump(mode="json"),
            **_project_snapshot(project),
        }
    if operation == "asset.list":
        return {"assets": [item.model_dump(mode="json") for item in project.list_assets()]}
    if operation == "asset.import":
        task = project.import_asset(
            str(_required(arguments, "source")),
            idempotency_key=_task_idempotency(envelope, operation),
        )
        ...
        return {
            **result,
            "asset": project.get_asset(imported_id).model_dump(mode="json"),
        }
    if operation == "sequence.short.create":
        sequence = project.create_short_from_bounds(
    ...
    if operation == "subtitle.list":
        documents_data = project.list_subtitle_documents(sequence_id=sequence_id)
        return {
            "documents": [
                {
                    **document.model_dump(mode="json"),
                    "segments": [
                        ...
                        for segment in project.list_subtitle_segments(document.id)
                    ],
                }
    ...
    if operation == "subtitle.segment.update":
        segment = project.update_subtitle_segment(
    ...
    if operation == "transcript.get":
        snapshot = project.inspect_transcript(
            sequence_id,
            document_id=(str(arguments["document_id"]) if arguments.get("document_id") else None),
        )
        return {"transcript": snapshot.model_dump(mode="json")}
    if operation == "transcript.edit.preview":
        edit = TranscriptEditRequest.model_validate(_required(arguments, "edit"))
        plan = project.preview_transcript_edit(
            edit,
            project.timeline(edit.sequence_id),
        )
        return {"plan": plan.model_dump(mode="json")}
    if operation == "transcript.edit.apply":
        plan = TranscriptEditPlan.model_validate(_required(arguments, "plan"))
        ...
        result = project.apply_transcript_edit(
            plan,
            project.timeline(plan.sequence_id),
        )
        return {"edit": result.model_dump(mode="json")}
    if operation == "audio.inspect":
        buses = project.list_audio_buses(sequence_id)
        return {
            "buses": [
                {
                    **bus.model_dump(mode="json"),
                    "effects": [
                        effect.model_dump(mode="json") for effect in project.list_audio_effects(bus.id)
                    ],
                }
                for bus in buses
            ]
    ...
    if operation == "audio.bus.update":
        ...
        bus = next(item for item in project.list_audio_buses(sequence_id) if item.id == bus_id)
        updated = project.save_audio_bus(bus.model_copy(update=dict(_required(arguments, "changes"))))
        return {"bus": updated.model_dump(mode="json")}
    if operation == "audio.effect.save":
        effect = project.save_audio_effect(AudioEffect.model_validate(_required(arguments, "effect")))
        return {"effect": effect.model_dump(mode="json")}
    if operation == "audio.effect.remove":
        project.remove_audio_effect(str(_required(arguments, "effect_id")))
        return {"removed": True}
    if operation == "preview.render":
        state = editor.state
        project.prepare_web_sequence(state)
    ...
    if operation == "task.list":
        return {"tasks": [item.model_dump(mode="json") for item in project.list_tasks()]}
    if operation == "task.status":
        task = project.get_task(str(_required(arguments, "task_id")))
        return {"task": task.model_dump(mode="json")}
    if operation == "task.start":
        task_command = _TASK_COMMAND_ADAPTER.validate_python(_required(arguments, "task_command"))
    ...
    if operation == "task.resume":
        task = project.resume_task(str(_required(arguments, "task_id")))
        return _task_result(project, task, float(arguments.get("timeout", 3600)))
    if operation == "web.import":
        asset = project.import_web_package(str(_required(arguments, "source")))
        return {
            "asset": asset.model_dump(mode="json"),
            "web_asset": project.inspect_web_asset(asset.id).model_dump(mode="json"),
        }
    if operation == "web.inspect":
        spec = project.inspect_web_asset(str(_required(arguments, "asset_id")))
        return {"web_asset": spec.model_dump(mode="json")}
    if operation == "web.clip.get":
        web_state = project.get_web_clip(str(_required(arguments, "clip_id")))
        return {"web_clip_state": web_state.model_dump(mode="json")}
    if operation == "web.clip.update":
        web_state = project.update_web_clip(
    ...
    if operation == "web.clip.diff":
        diff = project.diff_web_clip_update(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
            dict(_required(arguments, "updates")),
            expected_revision=(
    ...
    if operation == "web.clip.layout.select":
        web_state = project.select_web_layout(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
    ...
    if operation == "web.clip.keyframe.set":
        web_state = project.set_web_keyframe(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
    ...
    if operation == "web.clip.keyframe.remove":
        web_state = project.remove_web_keyframe(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
            str(_required(arguments, "layer_id")),
            str(_required(arguments, "field")),
    ...
    if operation == "web.clip.theme.update":
        web_state = project.update_web_theme(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
    ...
    if operation == "web.clip.data.update":
        web_state = project.update_web_data(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
    ...
    if operation == "web.clip.data.snapshot":
        web_state = project.update_web_data_from_file(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
            str(_required(arguments, "source")),
    ...
    if operation == "web.clip.lock.update":
        web_state = project.set_web_field_locks(
            str(_required(arguments, "sequence_id")),
            str(_required(arguments, "clip_id")),
            str(_required(arguments, "layer_id")),
    ...
    if operation == "web.clip.render":
        ...
        return _task_result(
            ...
            project.start_task(
                render_command,
                sequence_id=render_command.sequence_id,
                idempotency_key=_task_idempotency(envelope, operation),
            ),
            float(arguments.get("timeout", 3600)),
        )
    if operation == "web.clip.export":
        sequence_id = str(_required(arguments, "sequence_id"))
        web_export = project.export_web_clip(
            project.load_timeline(sequence_id),
            str(_required(arguments, "clip_id")),
    ...
    if operation == "web.batch.create":
        ...
        if records_value is not None:
            records = [dict(value) for value in cast(list[Mapping[str, object]], records_value)]
        else:
            records = project.read_web_variant_records(str(source_value))
        variants = project.create_web_variants(
            str(_required(arguments, "source_sequence_id")),
            str(_required(arguments, "clip_id")),
            records,
            {str(key): str(value) for key, value in dict(_required(arguments, "bindings")).items()},
    ...
    if operation == "web.asset.rebind":
        report = project.rebind_web_asset(
...
def execute_request(
    ...
    *,
    ...
    envelope = (
        request if isinstance(request, AutomationRequest) else AutomationRequest.model_validate(request)
    )
    ...
    if operation == "project.create":
        ...
        requested_name = str(_required(arguments, "name"))
        if envelope.request_id and (root / "project.mfp").is_file():
            with api.open_project(root, writable=True, cooperative=True) as project:
                if project.get_project().name != requested_name:
                    raise ValueError(
                        "Existing project does not match the retried project.create request"
                    )
                return project.execute_automation_request(
                    envelope.request_id,
                    operation,
                    arguments,
                    lambda: _project_snapshot(project),
                    atomic=True,
                )
        with api.create_project(root, requested_name) as project:
            return project.execute_automation_request(
                envelope.request_id,
                operation,
                arguments,
    ...
    with api.open_project(
        ...
        journal_id = envelope.request_id if writable else None
        task_backed = operation in {
            "asset.import",
            "export.sequence",
            "task.start",
            "task.resume",
        ...
        return project.execute_automation_request(
            ...
            operation,
            arguments,
            lambda: _dispatch_project_request(project, api, envelope, operation, arguments),
            atomic=not task_backed,
        )
