...
每个项目目录中的 `project.mfp` 是项目、素材、序列、轨道、片段、复合片段、转场、字幕、音频图、工作流、任务、任务事件、任务消费、自动化请求、协作事件、持久撤销组和导出设置的唯一持久化来源。当前 schema 版本为 43。42 个历史升级步骤由 `PROJECT_MIGRATIONS` 按源版本唯一登记，`ProjectSchemaMigrator` 只负责在最外层 SQLite 事务中依次执行；任一步没有准确推进到目标版本都会回滚整个升级并释放项目锁。版本 13 至 38 依次收口字幕、时间线转录、可编辑 Web Media、复合片段、视音频轨道配对、词级时间戳、导出历史、命名版本、任务幂等与租约、editable-media v5、素材文件夹和有序视觉效果链。版本 39 增加只追加的 `project_event`，把项目修订、写入路径、操作者、请求回执和操作结果放入同一提交；版本 40 把平台编码器实现迁移为中间策略；版本 41 增加 `undo_group` 和事件逆命令；版本 42 将全部项目、命名版本、任务、事件和快照统一迁移为最终的 `encoder_policy.mode=software|prefer_hardware` 与 `vendor=auto|nvidia|intel|amd|apple`；版本 43 把时间线撤销、重做和命名版本中的整条时间线快照迁移为只包含实际变化实体的双向补丁，保留并发合并语义，同时避免编辑历史随完整项目大小重复增长。MLT XML、分析 JSON、浏览器渲染缓存、代理、波形、SRT、FCPXML 和导出文件都是派生或交付产物，不反向充当编辑状态。
...
- editable-media 应用层没有总服务门面。`WebPackageService` 独占包读取、校验和不可变发布，`WebClipEditingService` 独占片段状态编辑，`WebBatchService` 独占结构化批量序列，`WebRebindService` 独占全项目换版计划与提交；`WebMediaServices` 只在组合根装配这四个对象，不转发业务方法。桌面端同样把浏览器/字段编辑、网页时间线、批量换版与导出分别交给 `WebController`、`WebTimelineController` 和 `WebDeliveryController`，三者只共享不可变的当前选择快照，QML 不再调用旧控制器成员。`Workspace.qml` 只编排工作区，快捷键集中在 `WorkspaceShortcuts.qml`。
...
- 每个项目写请求都带 `request_id`、`base_revision`、`actor` 和 `client_id`。服务在冲突判断前查询持久化回执；完全相同的重试返回第一次结果。不相交的过期写入依据连续事件日志自动重放，同一路径冲突返回相关事件。同步编辑与回执、项目事件在同一事务中提交；耗时任务使用请求 ID 派生的任务幂等键，终态任务、正式项目变化与 `task_consumption` 在结算事务中原子提交。精确重试复用原任务和已经提交的消费结果，不靠服务启动后的补偿扫描拼接半状态。
...
`TimelineEditor` 保留编辑命令和一次性提交，三方合并、波纹删除、通用规则及完整性校验分别由四个策略组件负责。普通片段换源、视觉效果新增/修改/重排/移除同样只在这里提交；换源会重新验证媒体类型和源区间，视觉效果则按片段内稳定 ID 与连续位置保存。`TimelineCompiler` 只编排编译过程，通用 XML 图、片段生产者、视频、音频和转场分别由五个图组件生成；片段生产者按持久化顺序把启用的色彩调整、高斯模糊和暗角转换为 MLT 滤镜，因此原生预览与最终导出读取同一效果链。预览可以选择代理，导出始终解析原片。`editable-media` v5 网页片段只通过 `WebRenderService` 进入媒体管道。media-sources v4 素材账本为每个素材明确指定 `browser`、`native-underlay` 或 `native-audio`，导入、编辑和换版都以该声明为准，不从扩展名或页面使用方式推断。`WebRenderService` 先由同一份 `WebClipState` 和媒体声明生成确定性的原生媒体时间线，再把项目状态和缓存事务交给进程级 `WebCaptureEngine`；后者按帧数、像素量、CPU 上限和可用物理内存复用有界 Chromium worker，通过验证器和生产捕获共用的 `await window.__hf.seek(seconds)`、同步布局与样式刷新边界并行捕获浏览器透明画面，不固定等待浏览器动画帧，并严格按帧序写入单个 FFmpeg 管道。FFmpeg 在该管道内解码原生视频底层、合成浏览器画面并混合原生音频，发布一个带透明画面和可选 FLAC 音轨的 MKV；`TimelineCompiler` 的视频与音频图都读取这个缓存路径，不再分别解析 HTML 和媒体文件。默认捕获后端是 Chrome CDP 的快速无损透明 PNG；`drawElementImage` 在每个 worker 上用原图 PSNR、模糊结构 PSNR、平均误差、局部最大误差、Alpha 完整性和预热后的稳定捕获耗时共同对照截图，只有全体一致通过才能启用，任一 worker 在正式生产期失效都会终止当前 FFmpeg 进程并从帧 0 用截图后端重建完整缓存。缓存生产者用 FFprobe 同时验证视频与音频编码、像素格式、尺寸、帧率、声道、采样率和时长，再以文件边缘指纹和原子 manifest 发布；消费者不再用“文件非空”猜测缓存完成状态。网页包导入扫描同时产生全包内容身份和逐文件完整性记录，复制线程在写入时复核原扫描结果，避免落盘后再次读取大型媒体。项目内发布目录是不可变内容边界，换版创建新目录；渲染校验提交回执、素材状态与发布哈希，不在每次编译时重算整个视频包。项目写锁与网页缓存写锁统一使用操作系统自动释放的 `ProcessFileLock`，不再依赖超时删除锁文件。MediaFlow Pro 不引入 HyperFrames 运行依赖或第二套网页动画时钟。属性编辑描述由领域清单与当前片段状态推导，CLI 与 QML 共同消费；参数关键帧与图层关键帧共用同一场景毫秒时钟。网页时间条拖动时只发瞬时浏览器预览，释放后由 `TimelineEditor` 原子持久化一次。时间线 QML 根组件只保存视口交互状态，工具栏、画布、轨道控制和六类图层分别渲染；它投影素材缩略图带和淡入淡出手柄，但不保存第二份片段状态。源监视器由应用层为单个素材生成临时 MLT 图，入出点在送入时间线时进入同一 `TimelineEditor`，截帧经真实解码注册为新素材。预览画布的缩放、平移和变换手柄只产生编辑意图，最终仍由 `TimelineEditor` 持久化；时间线波形按当前视口换算源素材区间，只读取和绘制可见峰值。C++ 插件只动态加载 MLT C API，把实时音频输出和顺序视频预解码分成两条消费者链路，以音频播放事件对齐绝对呈现节拍，并通过有界队列向 Qt Scene Graph 纹理供帧；实际延迟不会被重置后累加，追赶音频也不会积攒未来等待，它还不知道项目、任务或编辑规则。
...
领域、应用、基础设施、自动化和组合根全部进入严格 mypy 检查。架构测试固定层级依赖、显式仓储能力、连续迁移链、唯一自动化注册表、FFmpeg 唯一进程入口，以及已拆分上帝文件的尺寸和成员边界；网页媒体总服务不得重新获得业务方法，旧网页控制器成员也不得重新进入 QML。PySide/QML 边界通过真实 Qt 模型、控制器和 QML 冒烟测试验证。`scripts/ci/quality_plan.py` 是 CI 范围的唯一真源：文档与许可证只进入轻量维护链，平台无关代码先经过 Windows 预检并分配到四个确定、无重叠的核心分片，基础设施、桌面、QML、原生运行时、fixture、共享合同、依赖和 quality workflow 变化再并行执行 Windows 交互与离线完整链。Ubuntu 24.04 x64 通过 Xvfb/xcb 与 Weston headless 分别加载真实原生 QML 项，macOS runner 必须自证为 arm64。Windows、Linux、macOS 各自产生包含真实媒体、持久事件、撤销历史和附带字体字幕参考片的项目工件，另外两个平台分别重开、撤销、重做、重新链接、编辑、H.264 导出并逐帧对照，共六条迁移链。缓存分别绑定依赖运行时、Qt SDK 和原生源码身份，失败任务保留限时诊断产物；不会因运行时缺失而把这些边界降成跳过。需要 API 凭据的完整在线用户链只在仓库变量 `MEDIAFLOW_RUN_ONLINE_E2E=true` 且已配置真实密钥时运行。真实 ASR 模型和其它在线服务仍使用明确的 integration/slow 标记留在具备对应外部资源的环境执行。
