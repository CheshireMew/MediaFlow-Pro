...
当前阶段没有在真实 Linux 或 macOS 机器上执行完整桌面应用、原生预览和真实导出验收的计划，也不把缺少这两类实机验收视为跨平台改造未完成或任务阻塞。除非用户以后明确要求，Linux 与 macOS 支持默认通过平台无关的领域与应用测试、运行时合同、构建脚本检查、目标平台 CI 配置和代码审查来验收；Windows 本机继续覆盖可实际执行的完整桌面、原生预览和导出链路。
