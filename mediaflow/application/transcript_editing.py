...
class TranscriptEditingService:
    ...
    def inspect_transcript(
        ...
        *,
        ...
        segments = self.repository.list_subtitle_segments(document.id)
        words = self.repository.list_subtitle_words(
            document.id,
    ...
    def preview_plan(
        ...
        segments = self.repository.list_subtitle_segments(document.id)
        words = self.repository.list_subtitle_words(document.id)
        ...
        intervals = self._merged_intervals(raw_intervals)
        before_state = timeline.state
        after_state = timeline.preview_ripple_delete_intervals(intervals)
        ...
        subtitle_track_ids = {
            ...
            and any(
                ...
                for placement in self.repository.list_subtitle_placements(track.id)
            )
        }
        affected_track_ids.update(subtitle_track_ids)
        first_cut = intervals[0][0]
        ...
        plan = TranscriptEditPlan(
            ...
            impact=TranscriptEditImpact(
                ...
                removed_duration_frames=sum(end - start for start, end in intervals),
    ...
    def apply_plan(
        ...
        before_state = timeline.state
        before_segments = self.repository.list_subtitle_segments(plan.document_id)
        before_words = self.repository.list_subtitle_words(plan.document_id)
        selected_segment_ids = {
            item_id
        ...
        recovery = self.repository.create_project_version(
        ...
        with self.repository.transaction():
            ...
            temporary_timeline.apply_ripple_delete_intervals(intervals)
            after_words = [
                self._remap_word(
                    word,
                    intervals,
            ...
            self.repository.save_subtitle_segments(plan.document_id, after_segments)
            self.repository.save_subtitle_words(plan.document_id, retained_words)
            self.publication.write_document_srt(plan.document_id)
            after_state = self.repository.load_timeline(timeline.sequence_id)
        timeline.reload()
        after_segments = self.repository.list_subtitle_segments(plan.document_id)
        after_words = self.repository.list_subtitle_words(plan.document_id)

        def restore(state, segments, words) -> None:
            with self.repository.transaction():
                self.repository.save_subtitle_segments(plan.document_id, list(segments))
                self.repository.save_subtitle_words(plan.document_id, list(words))
        ...
        self.history.push(
            ProjectEditCommand(
                label="AI 转录剪辑",
                undo_action=lambda: restore(before_state, before_segments, before_words),
                redo_action=lambda: restore(after_state, after_segments, after_words),
            )
        )
        return TranscriptEditResult(
            ...
            removed_word_count=sum(
                not word.excluded and word.id in selected_word_ids
                for word in before_words
            ),
    ...
    def _source_transcript(
        ...
        if document_id:
            ...
            candidates = self.repository.list_subtitle_documents(sequence_id=sequence_id)
        matching = [
            document
