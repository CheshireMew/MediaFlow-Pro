...
from mediaflow.domain.web_manifest_primitives import WebScene
...
class ResolvedWebSceneMedia:
    scene: WebScene
    source_ids: tuple[str, ...]


def media_source_ids(media_sources: WebMediaSourcesManifest) -> set[str]:
    return {item.id for item in media_sources.sources}
...
def validate_media_bindings(
    ...
    resolve_media_bindings(manifest, media_sources, state)


def resolve_media_bindings(
    manifest: EditableMediaManifest,
    media_sources: WebMediaSourcesManifest,
    state: WebClipState,
) -> tuple[ResolvedWebSceneMedia, ...]:
    validate_clip_state_contract(manifest, state)
    sources_by_id = {source.id: source for source in media_sources.sources}
    resolved_scenes: list[ResolvedWebSceneMedia] = []
    ...
    for scene in manifest.scenes:
        ...
        resolved_scenes.append(
            ResolvedWebSceneMedia(
                scene=scene,
                source_ids=selected_ids,
            )
        )
    return tuple(resolved_scenes)
...
def validate_clip_state_contract(
    ...
    global_parameters = {item.descriptor.id for item in manifest.parameters if item.binding.scope == "global"}
    scene_parameters = {item.descriptor.id for item in manifest.parameters if item.binding.scope == "scene"}
    ...
    if state.variant is not None:
        ...
        if state.variant.width != variant.canvas.width or state.variant.height != variant.canvas.height:
    ...
    unknown_global_parameters = (set(state.parameters) | set(state.parameter_locks)) - global_parameters
    if unknown_global_parameters:
        raise ValueError(f"Editable global parameter state is invalid: {sorted(unknown_global_parameters)}")
    ...
    for scene_id, scene_state in state.scenes.items():
        ...
        referenced_layers = set(scene_state.layers) | set(scene_state.animations) | set(scene_state.locks)
        unknown_layers = referenced_layers - set(layers)
        if unknown_layers:
            raise ValueError(
                f"Editable state scene {scene_id} references unknown layers: {sorted(unknown_layers)}"
            )
        for layer_id, override in scene_state.layers.items():
            override_disallowed = override.changed_fields() - set(layers[layer_id].editable)
        ...
        for layer_id, fields in scene_state.locks.items():
            ...
            if lock_disallowed:
                raise ValueError(
                    f"Editable state locks {layer_id} contain non-editable fields: {sorted(lock_disallowed)}"
                )
        unknown_data = set(scene_state.data_snapshot.values) - data_fields
        if unknown_data:
            raise ValueError(
                f"Editable state scene {scene_id} references unknown data fields: {sorted(unknown_data)}"
            )
        unknown_scene_parameters = (
            set(scene_state.parameters) | set(scene_state.parameter_locks)
        ) - scene_parameters
        if unknown_scene_parameters:
            raise ValueError(
                f"Editable scene parameter state is invalid: {scene_id}/{sorted(unknown_scene_parameters)}"
            )
        unknown_parameter_animations = set(scene_state.parameter_animations) - set(parameters)
        if unknown_parameter_animations:
            raise ValueError(
                f"Editable parameter animation is invalid: {scene_id}/{sorted(unknown_parameter_animations)}"
            )
