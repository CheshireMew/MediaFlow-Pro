...
def migrate_stored_task_command(
    kind: str,
    ...
    *,
    ...
    if task_kind == TaskKind.IMPORT:
        ...
        if purpose not in {"media", "subtitle", "watermark"}:
            raise ValueError(f"Unsupported persisted import purpose: {purpose}")
        return ImportAssetCommand(
            source_path=str(values["source_path"]),
            purpose=cast(Literal["media", "subtitle", "watermark"], purpose),
    ...
    if task_kind == TaskKind.TRANSLATE:
        ...
        return TranslateDocumentCommand(
            document_id=document_id,
            target_language=target_language,
            mode=mode,
            workflow=workflow,
