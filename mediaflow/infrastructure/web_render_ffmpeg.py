...
def build_web_render_ffmpeg_command(
    ...
    for video_segment in target.native_media_plan.video_segments:
        ...
        next_input += 1
    audio_inputs, next_input = _append_native_audio_inputs(
        arguments,
        target,
    ...
    audio_filters, audio_map = _native_audio_filter_graph(
        target,
        audio_inputs,
...
def build_web_direct_h264_mux_command(
    target: WebRenderTarget,
    annex_b_path: Path,
    output_path: Path,
) -> list[str]:
    if not target.animated:
        raise ValueError("Direct H.264 muxing requires an animated target")
    if target.native_media_plan.video_segments:
        raise ValueError("Direct H.264 muxing cannot composite native-underlay video")
    fps = Fraction(target.fps_numerator, target.fps_denominator)
    duration_ms = Fraction(
        target.frame_count * target.fps_denominator * 1000,
        target.fps_numerator,
    ...
    audio_inputs, _next_input = _append_native_audio_inputs(
        arguments,
        target,
        next_input=1,
    )
    ...
    if audio_filters:
        arguments.extend(["-filter_complex", ";".join(audio_filters)])
    ...
    arguments.extend(
        [
            ...
            (
                "setts="
                f"pts=N*{fps.denominator}/{fps.numerator}/TB:"
                f"dts=N*{fps.denominator}/{fps.numerator}/TB:"
                f"duration={fps.denominator}/{fps.numerator}/TB"
            ),
            "-color_range",
            "tv",
            "-colorspace",
...
def _append_native_audio_inputs(
    arguments: list[str],
    target: WebRenderTarget,
    *,
    next_input: int,
) -> tuple[list[tuple[WebNativeAudioSegment, int]], int]:
    ...
    for audio_segment in target.native_media_plan.audio_segments:
        ...
        next_input += 1
    return audio_inputs, next_input


def _native_audio_filter_graph(
    target: WebRenderTarget,
    audio_inputs: list[tuple[WebNativeAudioSegment, int]],
    *,
    duration_ms: Fraction,
