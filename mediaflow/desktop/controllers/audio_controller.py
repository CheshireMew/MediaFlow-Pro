...
from mediaflow.desktop.presentation_timeline import (
...
class AudioController(ControllerFacet[CreativeControllerScope]):
    ...
    def selectedAudioBusId(self) -> str:
        return self._session.state.selection.audio_bus_id

    @Property(str, notify=selectionChanged)
    def selectedAudioEffectId(self) -> str:
        return self._session.state.selection.audio_effect_id

    @Property(dict, notify=audioMetricsChanged)
    def audioMetrics(self) -> dict:
        return self._session.state.presentation.audio_metrics
    ...
    def audioAnalysisRunning(self) -> bool:
        ...
        return any(
            ...
            and task.command.sequence_id == self._session.state.binding.active_sequence_id
    ...
    def selectAudioBus(self, bus_id: str) -> None:
        self._session.state.selection.audio_bus_id = bus_id
    ...
    def selectAudioEffect(self, effect_id: str) -> None:
        self._session.state.selection.audio_effect_id = effect_id
    ...
    def updateAudioBus(
        ...
        bus = next(
            item
            for item in self._session.state.binding.require_current().list_audio_buses(
                self._session.state.binding.active_sequence_id
            )
            if item.id == bus_id
        )
        self._session.state.binding.require_current().save_audio_bus(
            bus.model_copy(
                update={
    ...
    def addAudioBus(self, name: str) -> None:
        ...
        bus = self._session.state.binding.require_current().save_audio_bus(
            AudioBus(
    ...
    def addAudioEffect(self, bus_id: str, kind: str) -> None:
        ...
        effects = self._session.state.binding.require_current().list_audio_effects(bus_id)
        parameters: dict = {}
        ...
        effect = self._session.state.binding.require_current().save_audio_effect(
            AudioEffect(
                bus_id=bus_id,
                kind=effect_kind,
    ...
    def setAudioEffectEnabled(self, effect_id: str, enabled: bool) -> None:
        ...
        self._session.state.binding.require_current().save_audio_effect(
            effect.model_copy(update={"enabled": enabled})
        )
        self._session.projectors.audio.refresh_audio_effects()
        self._session.projectors.audio.invalidate_audio_metrics()
        self._session.projectors.timeline.schedule_preview_graph()
    ...
    def applyAudioEffectPreset(self, effect_id: str, preset_id: str) -> None:
        ...
        self._session.state.binding.require_current().save_audio_effect(validated)
        self._session.state.selection.audio_effect_id = effect_id
        self._session.projectors.audio.refresh_audio_effects()
        self._session.projectors.audio.invalidate_audio_metrics()
        self._session.projectors.timeline.schedule_preview_graph()
    ...
    def removeAudioEffect(self, effect_id: str) -> None:
        self._session._require_writable()
        self._session.state.binding.require_current().remove_audio_effect(effect_id)
        self._session.state.selection.audio_effect_id = ""
        self._session.projectors.audio.refresh_audio_effects()
        self._session.projectors.audio.invalidate_audio_metrics()
        self._session.projectors.timeline.schedule_preview_graph()
    ...
    def moveAudioEffect(self, effect_id: str, position: int) -> None:
        ...
        self._session.state.binding.require_current().save_audio_effect_chain(
            self._session.state.selection.audio_bus_id, effects
        )
        self._session.state.selection.audio_effect_id = effect_id
        self._session.projectors.audio.refresh_audio_effects()
        self._session.projectors.audio.invalidate_audio_metrics()
    ...
    def analyzeLoudness(self) -> None:
        self._session._require_writable()
        if not self._session.state.binding.active_sequence_id:
            raise RuntimeError("请先打开一个序列")
        self._session.tasks.start(
            AnalyzeLoudnessCommand(sequence_id=self._session.state.binding.active_sequence_id),
            sequence_id=self._session.state.binding.active_sequence_id,
        )
