...
def _resolve_reference(schema: dict[str, Any], reference: str) -> dict[str, Any]:
    if not reference.startswith("#/"):
        raise RuntimeError(
            f"editable-media v5 schema only allows local references: {reference}"
        )
    ...
    for encoded in reference[2:].split("/"):
        ...
        if not isinstance(current, dict) or key not in current:
            raise RuntimeError(
                f"editable-media v5 schema references a missing location: {reference}"
            )
        current = current[key]
    if not isinstance(current, dict):
        raise RuntimeError(
            f"editable-media v5 schema reference is not an object: {reference}"
        )
    return current
...
def _validate_node(
    ...
    if "enum" in node and all(
        ...
        errors.append(f"{location} is not an allowed editable-media v5 value")
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            if additional is False:
                errors.append(f"{location}.{key} is not an editable-media v5 field")
...
def validate_editable_media_document(document: object) -> None:
    ...
    if errors:
        raise ValueError(
            "editable-media v5 schema validation failed:\n- "
            + "\n- ".join(errors)
        )
