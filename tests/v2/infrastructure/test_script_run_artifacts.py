...
def test_script_run_retention_keeps_failures_interruptions_and_latest_success(
    ...
    previous_app_root = os.environ.get("MEDIAFLOW_APP_ROOT")
    ...
    with verification_run("policy", managed_root=managed_root) as first_success:
        ...
        assert Path(os.environ["MEDIAFLOW_APP_ROOT"]).is_relative_to(
            first_success
        )
    ...
    assert os.environ.get("MEDIAFLOW_APP_ROOT") == previous_app_root
