(() => {
  ...
  let state = {
    scenes: {}, theme: {}, theme_bindings: {}, variant: {},
  ...
  function clearRuntimeStyles() {
    nodes.forEach((node) => {
      [
        ...
        "rotate", "opacity", "z-index", "visibility",
  ...
  function buildSceneState(scene, variant, previous = null) {
    const dataDefaults = Object.fromEntries(
      (manifest.data_fields || []).map((item) => [item.id, clone(item.default)])
    );
    return {
      layers: mergeLayerMaps(variant.layers || {}, scene.layers || {}),
      animations: clone(previous?.animations || scene.animations || {}),
      data: {
        ...dataDefaults,
        ...(scene.data || {}),
        ...(previous?.data || {}),
      },
      locks: clone(previous?.locks || scene.locks || {}),
    };
  }
  ...
  function setVariant(variantId) {
    ...
    const nextScenes = Object.fromEntries(
      manifest.scenes.map((scene) => [
        scene.id,
        buildSceneState(scene, variant, state.scenes?.[scene.id]),
      ])
    );
    return setState({
      ...state,
  ...
  function seekSeconds(seconds) {
    ...
    pause();
    return setTime(value * 1000) / 1000;
  }
